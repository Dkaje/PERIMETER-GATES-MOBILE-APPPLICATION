        ...
        if (!preg_match($naming, $lname)) {
            ...
            if (mysqli_num_rows($query) > 0) {
                ...
                if (mysqli_num_rows($query) > 0) {
                    ...
                    $select = "SELECT contact FROM supplier WHERE contact='$phone'";
                    $query = mysqli_query($db, $select);
                    if (mysqli_num_rows($query) > 0) {
                        $response["success"] = 0;
                        ...
                        fclose($fp);
                        $values = $hits[0];
                        $supplier_id = $values . $year;
                        $sql = "INSERT INTO supplier(supp_id,fname,lname,username,contact,email,password)
    VALUES('$supplier_id','$fname','$lname','$username','$phone','$email','$password')";
                        if (mysqli_query($db, $sql)) {
                            $response["success"] = 1;
                            $response["message"] = "Your Entry is " . $supplier_id . " Account cretaed successfully";
                            echo json_encode($response);
                            mysqli_close($db);
                        } else {
                            $response["success"] = 0;
