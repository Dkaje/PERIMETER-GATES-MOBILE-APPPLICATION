...
if (strlen($_SESSION['username']) == 0) {
    ...
    if (isset($_GET['inid'])) {
        $id = $_GET['inid'];
        $status = 2;
        $sql = "update customer set status=:status WHERE cust_id=:id";
        $query = $con->prepare($sql);
        $query->bindParam(':id', $id, PDO::PARAM_STR);
        $query->bindParam(':status', $status, PDO::PARAM_STR);
        $query->execute();
        header('location:custApproved');
    }
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $status = 1;
        $sql = "update customer set status=:status,remarks='' WHERE cust_id=:id";
        $query = $con->prepare($sql);
        $query->bindParam(':id', $id, PDO::PARAM_STR);
        $query->bindParam(':status', $status, PDO::PARAM_STR);
        $query->execute();
    ...
    if (isset($_GET['idi'])) {
        $idi = $_GET['idi'];
        $status = 1;
        $sql = "update customer set status=:status,remarks='' WHERE cust_id=:idi";
        $query = $con->prepare($sql);
    ...
    if (isset($_POST['rejectBtn'])) {
        ...
        $status = 2;
        $sql = "update customer set status=:status,remarks=:remarks WHERE cust_id=:idi";
        $query = $con->prepare($sql);
    ...
    <div class="content-wrapper">
        <div class="container">
            <div class="row pad-botm">
                <div class="col-md-12">
                    <h5>
                        <a href="manageCus"> <button class="btn btn-wraning"> Manage Customers</button></a>
                        <a href="custPending"> <button class="btn btn-secondary"> Pending</button></a>
                        <a href="custApproved"> <button class="btn btn-success"> Approved</button></a>
                        <a href="custRejected"> <button class="btn btn-danger"> Rejected</button></a>
                    </h5>
                </div>
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                        <?php $sql = "SELECT * from customer where status=1";
                                            $query = $con->prepare($sql);
                                            $query->execute();
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td class="center">
                                                        ...
                                                        <li>
                                                            <form method="post">
                                                                <input type="hidden" name="identity"
                                                                    value="<?php echo htmlentities($result->cust_id); ?>" />
                                                ...
                                                <a href="custApproved?idi=<?php echo htmlentities($result->cust_id); ?>"
                                                    onclick="return confirm('Are you sure you want to Accept this Customer?');"" >  <button class="
                                                ...
                                                <a href="custApproved?id=<?php echo htmlentities($result->cust_id); ?>"
                                                    onclick="return confirm('Are you sure you want to activate this Customer?');"><button
