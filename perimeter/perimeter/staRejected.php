...
if (strlen($_SESSION['username']) == 0) {
    ...
    if (isset($_GET['inid'])) {
        ...
        $query->execute();
        header('location:staRejected');
    }
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $status = 1;
        $sql = "update staff set status=:status,remarks='' WHERE staff_id=:id";
        $query = $con->prepare($sql);
        $query->bindParam(':id', $id, PDO::PARAM_STR);
        $query->bindParam(':status', $status, PDO::PARAM_STR);
        $query->execute();
    ...
    if (isset($_GET['idi'])) {
        $idi = $_GET['idi'];
        $status = 1;
        $sql = "update staff set status=:status,remarks='' WHERE staff_id=:idi";
        $query = $con->prepare($sql);
    ...
    if (isset($_POST['rejectBtn'])) {
        ...
        $status = 2;
        $sql = "update staff set status=:status,remarks=:remarks WHERE staff_id=:idi";
        $query = $con->prepare($sql);
    ...
    <div class="content-wrapper">
        <div class="container">
            <div class="row pad-botm">
                <div class="col-md-12">
                    <h5>
                        <a href="manageSta"> <button class="btn btn-wraning"> Manage Staff</button></a>
                        <a href="staPending"> <button class="btn btn-secondary"> Pending</button></a>
                        <a href="staApproved"> <button class="btn btn-success"> Approved</button></a>
                        <a href="staRejected"> <button class="btn btn-danger"> Rejected</button></a>
                    </h5>
                </div>
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                        <?php $sql = "SELECT * from staff where status=2";
                                            $query = $con->prepare($sql);
                                            $query->execute();
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td class="center"><?php echo htmlentities($result->staff_id); ?></td>
                                            ...
                                            <td class="center">
                                                        ...
                                                        <li>
                                                            <form method="post">
                                                                <input type="hidden" name="identity"
                                                                    value="<?php echo htmlentities($result->staff_id); ?>" />
                                                ...
                                                <a href="staRejected?idi=<?php echo htmlentities($result->staff_id); ?>"
                                                    onclick="return confirm('Are you sure you want to Accept this staff?');"" >  <button class="
                                                ...
                                                <a href="staRejected?id=<?php echo htmlentities($result->staff_id); ?>"
                                                    onclick="return confirm('Are you sure you want to activate this staff?');"><button
